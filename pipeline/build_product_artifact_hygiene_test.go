...
func TestBuildProductCommitIfDirtySkipsBinaryArtifact(t *testing.T) {
	...
	// PR #411 finding #2: the runtime binary-artifact exclusion must NOT touch
	// the tracked .gitignore — a runtime write there is itself an out-of-scope
	// tree change that VerifyMilestone would FAIL, defeating the purpose of
	// skipping the binary. #640 C6: it must not persist in .git/info/exclude
	// either — a persistent `/goblin` line matched a DIRECTORY forever, so a
	// later goblin/goblin.go was never staged. The exclusion is per-invocation.
	if excl, err := os.ReadFile(filepath.Join(dir, ".git", "info", "exclude")); err == nil && strings.Contains(string(excl), "goblin") {
		t.Errorf("CommitIfDirty persisted the binary-artifact exclude into .git/info/exclude — a later directory of the same name is never staged (issue #640 C6):\n%s", excl)
	}
