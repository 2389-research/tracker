...
func (e *Engine) executeNode(ctx context.Context, s *runState, currentNodeID string, execNode *Node) (*Outcome, TraceEntry, error) {
	e.emit(PipelineEvent{
		...
		Timestamp: time.Now(),
		RunID:     s.runID,
		NodeID:    currentNodeID, NodeKind: execNode.Handler, AttemptNo: s.cp.RetryCount(currentNodeID) + 1,
...
func (e *Engine) applyOutcome(s *runState, currentNodeID string, outcome *Outcome) {
	...
	// Absorb any child-propagated validation overrides (subgraph/manager_loop)
	// into the run's sticky + checkpoint lists (#535 dedup lives in the helper).
	e.applyChildOverrides(s, currentNodeID, outcome.ChildOverride)
}
...
func (e *Engine) handleRetryWithinBudget(ctx context.Context, s *runState, currentNodeID string, execNode *Node, traceEntry *TraceEntry, policy *RetryPolicy) (string, bool, *EngineResult, error) {
	...
	e.emit(PipelineEvent{
		...
		Timestamp: time.Now(),
		RunID:     s.runID,
		NodeID:    currentNodeID, NodeKind: execNode.Handler, AttemptNo: s.cp.RetryCount(currentNodeID),
