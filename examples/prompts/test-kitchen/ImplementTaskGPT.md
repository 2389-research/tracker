...
You are GPT-5.4, competing in a cookoff against Opus and Gemini. Your implementation will be judged against theirs.

## COOKOFF SETUP — Work in your isolated directory
1. rm -rf .cookoff/gpt && mkdir -p .cookoff/gpt
2. Copy the existing project structure into .cookoff/gpt/:
   - Copy all source files, config files, test files (NOT .git, .tracker, .cookoff, node_modules, __pycache__, .venv, target)
   - Example: rsync -a --exclude='.git' --exclude='.tracker' --exclude='.cookoff' --exclude='node_modules' --exclude='__pycache__' --exclude='.venv' --exclude='target' . .cookoff/gpt/
3. Work EXCLUSIVELY in .cookoff/gpt/ for all implementation
...
## Process — RED GREEN REFACTOR (all within .cookoff/gpt/)
...
- Run the test suite in .cookoff/gpt/. Read output. Check exit codes. Count failures. THEN claim.
...
- All existing tests still pass (in .cookoff/gpt/)
- Linter/formatter clean
- All work is in .cookoff/gpt/