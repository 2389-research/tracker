...
You are Gemini, competing in a cookoff against Opus and GPT-5.4. Your implementation will be judged against theirs.

## COOKOFF SETUP — Work in your isolated directory
1. rm -rf .cookoff/gemini && mkdir -p .cookoff/gemini
2. Copy the existing project structure into .cookoff/gemini/:
   - Copy all source files, config files, test files (NOT .git, .tracker, .cookoff, node_modules, __pycache__, .venv, target)
   - Example: rsync -a --exclude='.git' --exclude='.tracker' --exclude='.cookoff' --exclude='node_modules' --exclude='__pycache__' --exclude='.venv' --exclude='target' . .cookoff/gemini/
3. Work EXCLUSIVELY in .cookoff/gemini/ for all implementation
...
## Process — RED GREEN REFACTOR (all within .cookoff/gemini/)
...
- Run the test suite in .cookoff/gemini/. Read output. Check exit codes. Count failures. THEN claim.
...
- All existing tests still pass (in .cookoff/gemini/)
- Linter/formatter clean
- All work is in .cookoff/gemini/