...
next_sprint_id() {
  # awk does the decimal parse ($1+0 reads 008 as 8) and the max, and flags
  # the first non-numeric id; the shell only pads.
  last=$(awk -F '\t' 'NR>1 && $1 !~ /^[0-9]+$/ { print "BAD " $1; exit }
                     NR>1 && $1+0 > max { max = $1+0 }
                     END { if (max) print max }' .ai/ledger.tsv 2>/dev/null || true)
  case "$last" in
    '')    printf '001' ;;
