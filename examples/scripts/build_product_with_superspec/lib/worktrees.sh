...
# reachable). Records the phase base sha (.ai/build/milestone-start-sha) the
# phase's gate scopes its tests/lint against; merge_streams leaves it alone.
...
# current branch, fold the streams' traceability overlays into the master
# (lib/traceability.sh), and only once BOTH succeeded tear down the
# worktrees + branches. Order matters: a malformed overlay or a hook-rejected
# fold commit must fail while every worktree/branch is still in place, so
# the MergeConflict gate's "retry" can re-enter (a retry skips branches
# already in HEAD). On a conflict (5c): the merge is aborted (`|| true` — an
# already-aborted merge has nothing to abort, and that used to exit 128),
# the conflicting paths are printed, nothing is torn down, and the function
# returns 1 — the workflow routes that to the MergeConflict gate, never to
# accept. The phase base sha (.ai/build/milestone-start-sha) is NOT touched
# here: the gate that follows scopes its tests/lint to the changes since the
# base setup_stream_worktrees recorded — advancing it to HEAD before the
# gate made verify.sh see zero changed files and `--new-from-rev HEAD` lint
# nothing. finish_gate (lib/gates.sh) advances it on PASS.
merge_streams() {
  _phase=$1; shift
  mkdir -p .ai/build
  ...
  merge_traceability_overlays "$_phase" || {
    echo "Traceability fold failed — fix the overlay(s) in the workdir and commit, then choose 'retry' at the gate. Worktrees and branches are untouched."
    return 1
  }
  for _s in "$@"; do
