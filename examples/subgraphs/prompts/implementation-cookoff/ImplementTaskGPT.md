...
## WORKING DIRECTORY
All your work MUST happen inside `.cookoff/gpt/` relative to the project root.
1. mkdir -p .cookoff/gpt
2. Copy the project source files (not .git, not .tracker, not .cookoff) into .cookoff/gpt/ if not already there
3. Do all implementation work inside .cookoff/gpt/
...
- Run the test suite inside .cookoff/gpt/. Read output. Check exit codes. Count failures. THEN claim.
