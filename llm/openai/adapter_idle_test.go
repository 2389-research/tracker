...
func TestStream_IdleDeadlineSurfacesRetryableError(t *testing.T) {
	...
	srv := hangingSSEServer(t, "data: {\"type\":\"response.created\"}\n\n", release)
	defer srv.Close()
	defer close(release)

	a := New("k", WithBaseURL(srv.URL), WithStreamIdleTimeout(150*time.Millisecond))
	ch := a.Stream(context.Background(), &llm.Request{Model: "gpt-5", Messages: []llm.Message{llm.UserMessage("hi")}})

	streamErr, finished := drainUntilClose(t, ch)
	if streamErr == nil {
	...
	if finished {
		t.Fatal("idle stream must not emit a finish event")
	}
}
