...
- `build_product` has two post-build gates: `EscalateVerification` (default `abandon` → `AbortRun`; red FinalBuild, failed FinalSpecCheck, reviewer/CheckReviewsComplete failure, SynthesizeReviews/ApplyReviewFixes catch-alls) and `EscalateReview` (default `accept`, `override: true`; ONLY `CheckReviewFixBudget` exhausted and `FinalCommit`'s fallback). Route new red/unverified failures to the former; pre-build agent failures (ReadSpec/Decompose) go to `AbortRun`. Every gate's `abandon` routes to `AbortRun` — a gate edge to `Done` ends the run `success`.
