...
- `tracker init <name>` — copies a built-in workflow to cwd for customization, together with its `prompts/<name>/` + `scripts/<name>/` sidecar files (so the copied `.dip`'s relative `*_file` paths resolve on disk). Refuses to overwrite any of them.
