...
- **Checkpoint (#559, related, partially addressed).** `checkpoint.json` lives at `<workDir>/.tracker/runs/<runID>/` — workDir-resident, so tool-reachable — and unlike the activity log it is **authoritative for resume** (`EdgeSelections` picks the next edge, `Context` is restored). An unjailed same-UID tool can therefore tamper it to inject control flow / context on the next `tracker -r`. Shipped mitigation: `writeFileAtomic` now `O_NOFOLLOW`s the temp write, closing the *redirect-the-runtime's-own-write* symlink vector. NOT yet closed: the content-tamper vector itself — relocating (à la #213) vs integrity-verify is an open design call. The real boundary for an untrusted tool node is the `writable_paths` jail (a jailed tool can't write `.tracker/`).
