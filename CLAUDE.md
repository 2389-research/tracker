...
- Gateway kind dispatch via `TRACKER_GATEWAY_KIND` / `--gateway-kind` (default `cf-aig`). For `cf-aig` the suffixes are `/anthropic`, `/openai`, `/google-ai-studio`, `/compat`. For `bedrock` Anthropic gets `""` (SDK appends `/v1/messages`), OpenAI/Gemini get `/v1`, and `openai-compat` refuse-routes via `ErrGatewayRouteRefused` (use `tracker.ResolveProviderBaseURLStrict` to surface the refusal). Unknown kinds fail closed.
...
- Local preview: `cd site && hugo server` (port 1313). `baseURL` is `/tracker/`; `uglyURLs = true` keeps the `/tracker/<name>.html` shape.
