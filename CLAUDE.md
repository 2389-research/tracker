...
- `tracker init <name>` — copies a built-in workflow to cwd for customization, together with every sidecar its `*_file` directives reference (derived from the parsed IR, e.g. `prompts/<name>/…` + `scripts/<name>/…`; superspec gets the shared `prompts/build_product/SpecLint.md`), so the copied `.dip`'s relative paths resolve on disk. Refuses to overwrite any of them.
