...
- `TRACKER_GATEWAY_URL` env var or `--gateway-url` CLI flag synthesizes per-provider base URLs for Cloudflare AI Gateway. Provider-specific `*_BASE_URL` always wins over the gateway URL.
