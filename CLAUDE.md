...
- A failed node that HAS conditional edges but none matching `fail` no longer dead-stops with `no matching edges` when a fallback exists (#653): `advanceToNextNode` → `unmatchedFailureCascade` (`pipeline/engine_failure_cascade.go`) runs steps 3–5 on the typed `noMatchingEdgesError`, emitting `conditional_fallthrough` (with the missed guards) + `decision_edge` with `edge_priority = fallback` (`EdgePriorityFallback`) and recording the hop via `SetEdgeSelection` so resume replays it. A failed node with conditional edges AND an unconditional edge still takes the unconditional edge (intentional routing, as before).
