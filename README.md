...
> **What's new in v0.45.0** (2026-07-14): **escape-aware condition parsing, reconciled with dippin v0.49**. Building on v0.44.0's #444 fix, logical splitting (`||`/`&&`) and comparison-operator discovery (`=`/`==`/`!=` and the word operators) now both ignore content inside double quotes; exactly one surrounding quote pair is stripped from every RHS and `\"`/`\\` are decoded inside it. A quote after an odd run of backslashes is escaped, an even run closes the operand, and an **unmatched double quote is a loud error** — carrying the original expression and, at load, the `.dip` source location — instead of a silent mis-parse. Semantic validation applies the same quote-aware grammar to **every logical branch independently**, so a malformed later branch is rejected even when an earlier branch would short-circuit ([#444](https://github.com/2389-research/tracker/issues/444) reconciliation, [#470](https://github.com/2389-research/tracker/pull/470)). dippin-lang is bumped v0.48.0 → v0.49.0 (library, CI CLI, and doctor pin move together). Separately, `tracker-swebench`'s 200-line `main()` was decomposed under the complexity gate and its containers bounded to 1 CPU ([#469](https://github.com/2389-research/tracker/pull/469)), and the project gains a rolling [`ROADMAP.md`](ROADMAP.md) + website Roadmap page. No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
> **Previously in v0.44.0** (2026-07-13): **engine-correctness hardening + a real, enforced complexity gate**. Five audit fixes: the condition evaluator no longer misroutes on `||`/`&&` inside quoted values ([#444](https://github.com/2389-research/tracker/issues/444)), human-gate timeouts cancel the interviewer instead of leaking its goroutine ([#446](https://github.com/2389-research/tracker/issues/446)), claude-code error classification is anchored so benign output can't flip a fail into an infinite retry ([#447](https://github.com/2389-research/tracker/issues/447)), the interview-result handler fails the node instead of panicking ([#448](https://github.com/2389-research/tracker/issues/448)), and `Outcome.Status` is now typed `TerminalStatus` so a typoed status can't compile ([#445](https://github.com/2389-research/tracker/issues/445)). The canonical `make ci` **complexity gate is now real and CI-enforced** via a shrink-only baseline ratchet — it was permanently red and unrun before ([#468](https://github.com/2389-research/tracker/issues/468)). dippin-lang is bumped to v0.48.0 and all example pipelines restored to A grade. No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
> **Previously in v0.43.0** (2026-07-09): **an autonomous spec-forge loop for `build_product`, plus a packed-run safety fix**. When `SpecLint` fails, `build_product` no longer dead-ends at a human: a `ForgeSpec` agent hardens `SPEC.md` (reconciling — never deleting — requirements, and elaborating only gaps with a cited seed span), a `CheckSpecFidelity` oracle proves no requirement was silently dropped, and the loop re-lints until clean — capped at 3 attempts and failing **closed** (so `--auto-approve`/`--autopilot` can't ship a broken spec). The forge diff + rulings are surfaced at `ApprovePlan` before any build spend, and `SpecLint` gains a "buildable substance" check so a too-thin spec is caught. This hardens **structural coherence**; it doesn't certify semantic consistency. Separately, a packed `.dipx` run that references `${graph.workflow_dir}` now **fails loud before any node runs** instead of silently degrading to `. "/scripts/…"` and aborting under `set -eu` ([#430](https://github.com/2389-research/tracker/issues/430)). No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
> **Previously in v0.42.0** (2026-07-02): **`build_product` milestone gate-loop fixes** — from a post-mortem of a run that wedged and couldn't self-recover. The lint gate is now **milestone-scoped** like `go test` (`--new-from-rev`), so a milestone no longer fails on earlier milestones' pre-existing lint debt ([#436](https://github.com/2389-research/tracker/issues/436)); **FixMilestone** now receives the failing gate's real stdout and re-runs the exact gate instead of being blind to it ([#437](https://github.com/2389-research/tracker/issues/437)); the structural-outputs gate is **scoped to built milestones** so the early `accept` ship path stops flagging not-yet-built directories ([#439](https://github.com/2389-research/tracker/issues/439)) and now **parses declared paths** (first backticked token) instead of tokenizing prose into phantom "missing" files ([#440](https://github.com/2389-research/tracker/issues/440)). There's now an **in-band escape hatch for lint failures** (`.ai/milestones/known_lint_failures` → `golangci-lint --exclude`, surfaced at escalation) ([#441](https://github.com/2389-research/tracker/issues/441)), a **louder warning** when `golangci-lint` is absent ([#442](https://github.com/2389-research/tracker/issues/442)), and the milestone retry cap now runs **exactly N attempts** ([#443](https://github.com/2389-research/tracker/issues/443)). Verified that `CheckMilestoneOutputs` already probes the working tree — no git-tree bug ([#438](https://github.com/2389-research/tracker/issues/438)). No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
> **Previously in v0.41.0** (2026-07-01): **`build_product` review-fidelity hardening plus a batch of engine-resilience primitives**. On the review side: cross-review lanes now read a **bounded diff** and tier their models so the panel stops being ~90% of run tokens ([#418](https://github.com/2389-research/tracker/issues/418)), cheaply-verified agent nodes drop to a cheaper tier ([#419](https://github.com/2389-research/tracker/issues/419)), reviewers now catch **self-ratifying generated tests** and unasserted "normative" constants that used to pass GREEN ([#417](https://github.com/2389-research/tracker/issues/417)), and a new **contract-fidelity** check flags fake≠real seam divergence at external boundaries ([#416](https://github.com/2389-research/tracker/issues/416)). On the engine side: opt-in **content-hash node-output memoization** replays already-green nodes across loop restarts (`params: { memoize: true }`, agent-nodes only, never for `writable_paths` nodes) ([#421](https://github.com/2389-research/tracker/issues/421)); **sleep-aware budgets** exclude OS-suspend spans from wall/stall accounting via monotonic-clock accounting ([#422](https://github.com/2389-research/tracker/issues/422) part A); and a **device-node hygiene preflight** plus artifact-repo health check + reattach harden long/suspended runs ([#423](https://github.com/2389-research/tracker/issues/423)). `build_product` also derives **spec-contract artifacts** — forced ambiguity rulings and a behavioral-contract checklist cited by Verify ([#306](https://github.com/2389-research/tracker/issues/306)). Housekeeping: dead middleware abstractions removed ([#394](https://github.com/2389-research/tracker/issues/394)), silent error-swallowing fixed in the Gemini translator / TUI review / CLI env load ([#397](https://github.com/2389-research/tracker/issues/397)), and more example `.dip` files reach A grades under `dippin` v0.43 ([#335](https://github.com/2389-research/tracker/issues/335)). No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
> **Previously in v0.40.2** (2026-06-24): **human gates finally display their `prompt:` body, and `build_product`'s plan-approval gate shows you the actual plan**. The `wait.human` handler built the gate prompt from `label:` alone and silently dropped a node's multi-line `prompt:` for freeform/choice/yes_no modes — so authored prompt bodies (and any `${ctx.tool_stdout}` interpolation the gate relied on to surface live state, including v0.40.1's [#407](https://github.com/2389-research/tracker/issues/407) `EscalateMilestone` "Verify currently" block) never rendered at runtime; `resolveHumanPrompt` now appends the expanded prompt body to the label (label-only gates unchanged). Building on that, the `build_product` `ApprovePlan` gate — which used to show only its one-line label, leaving an operator nothing to review but `approve` — gains a `ShowPlan` tool node that reads the planner's output (`.ai/decisions/milestones.md` and the spec `requirement-coverage.md` table, including the `COVERAGE_GAPS` count) into `ctx.tool_stdout`, interpolated under a `## Plan under review` heading so the full plan renders in the fullscreen review modal. `Decompose` success now routes `-> ShowPlan -> ApprovePlan`; the approve/adjust/reject routes are unchanged. No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
