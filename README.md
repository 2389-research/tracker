...
> **What's new in v0.39.2** (2026-06-15): **two prompt-fidelity fixes from case-study round 7**. `${ctx.tool_stdout}` and `${ctx.tool_stderr}` interpolated in agent prompts now render as fenced ` ```text ` blocks under their own headings instead of pasting raw tool output inline ([#352](https://github.com/2389-research/tracker/issues/352) item 3). `build-context.md` is now refreshed at each `MarkMilestoneDone` with the milestone's changed files, so downstream agents see which files are actively being worked rather than only the initial Setup entry-point list ([#351](https://github.com/2389-research/tracker/issues/351) item 3). No breaking changes; core pipelines hold A grades on `dippin doctor`.
>
