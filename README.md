...
> **What's new in v0.24.0** (2026-04-24): **two budget-bypass P1 fixes** — `subgraph` and `stack.manager_loop` nodes no longer ignore `--max-tokens` / `--max-cost` (child engines now inherit the parent's `BudgetGuard` + baseline usage, and their spend rolls up via `Outcome.ChildUsage` so parent-level guards fire between nodes); ACP backend surfaces approximate per-prompt token usage from rune counts across assistant text, reasoning chunks, and tool-call argument/result payloads (ACP protocol exposes no native usage, so the estimator is heuristic — see `docs/architecture/backends.md`); `claude-code` and `acp` backends now correctly populate `SessionResult.Provider` and thread `cfg.Model` into `TokenTracker.AddUsage` so per-provider `ProviderTotals` and library `Result.Cost.ByProvider` are no longer silently $0 / bucketed as `"unknown"`; `llm.EstimateCost` warns once per unknown model instead of silently returning $0; dippin-lang v0.23.0 bump with adapter-side wiring for `WorkflowDefaults.ToolCommandsAllow`.
>
