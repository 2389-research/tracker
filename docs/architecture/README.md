...
| [`llm.md`](./llm.md) | `llm.Client`, middleware stack, `TokenTracker`, cost estimator (base prices from `dippin-lang/pricing` #558, tracker cache overlay), and the four provider adapters (`anthropic`, `openai`, `google` / Gemini, `openaicompat`). |
