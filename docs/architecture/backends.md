...
- **Claude-code**: the NDJSON `result` message includes token usage *and* cache-token breakdown (`cache_read_input_tokens`, `cache_creation_input_tokens`); `parseMessage` builds the `SessionResult.Usage`, populates `CacheReadTokens` / `CacheWriteTokens` (as `*int`) when non-zero, sets `SessionResult.Provider = "claude-code"`, and `CodergenHandler.trackExternalBackendUsage` passes both (with `runCfg.Model`) to `TokenTracker.AddUsage("claude-code", usage, model)`. `TotalTokens` covers the full billable footprint (fresh input + output + cache tokens). `llm.EstimateCost` reads the cache fields separately and prices cache reads at 10% and cache writes at 25% of the input rate (Anthropic convention). Model normalization through the catalog means dated IDs price correctly.
