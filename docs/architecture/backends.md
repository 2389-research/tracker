...
- **ACP**: the ACP protocol does not currently expose per-prompt token usage in a way tracker consumes. Turn counts and tool call counts are recorded; costs are underreported.

This is tracked in CHANGELOG.md v0.20.0 — the "cost accounting fix" applies to native and claude-code; ACP usage improvements are follow-on work.
