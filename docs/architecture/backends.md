...
- **Claude-code**: the NDJSON `result` message includes token usage *and* cache-token breakdown (`cache_read_input_tokens`, `cache_creation_input_tokens`); `storeResult` builds the `SessionResult.Usage`, populates `CacheReadTokens` / `CacheWriteTokens` (as `*int`) when non-zero, sets `SessionResult.Provider = "claude-code"`, and `CodergenHandler.trackExternalBackendUsage` passes both (with `runCfg.Model`) to `TokenTracker.AddUsage("claude-code", usage, model)`. `TotalTokens` stays fresh input + output to match the convention in `llm/anthropic/translate_response.go` — cache tokens are priced independently by `llm.EstimateCost` via the separate `*int` fields (tracker's cache overlay: 10% of input for cache reads, 1.25× on Anthropic for cache writes / 0 elsewhere, #558). Dated/aliased IDs resolve through `dippin-lang/pricing`'s `Lookup` (which base prices come from), so they price correctly.
...
Cache-token tuning (`TRACKER_ACP_CACHE_READ_RATIO`): the ACP heuristic defaults to pricing all estimated input as fresh — conservative (never under-reports) but up to ~3× high for bridges (like `claude-agent-acp`) that keep a stable prompt cache. Operators who know their workload can set `TRACKER_ACP_CACHE_READ_RATIO` to a value in `(0, 1]` to route that fraction of the estimated input to `Usage.CacheReadTokens`, which `llm.EstimateCost` prices at 10% of the input rate. Typical values for stable-context Claude workloads: `0.5`–`0.8`. Out-of-range or non-numeric values log a one-time warning and are ignored. The native claude-code backend reads real cache-token counts from the NDJSON `result` envelope (no heuristic needed); the tuning knob is ACP-only because ACP has no equivalent surface. The `ACPUsageMarker{Estimated:true}` tag on `Usage.Raw` is the in-memory handoff: `buildSessionStats` calls `extractEstimateMarker(r.Usage.Raw)` to copy `Estimated` + `EstimateSource` onto `SessionStats` before `Usage.Raw` is dropped by later `llm.Usage.Add` calls. From `SessionStats` the flag flows forward: `Trace.AggregateUsage` OR-propagates it into `ProviderUsage.Estimated` and the summary-level `UsageSummary.Estimated`; the CLI "Tokens by Provider" table suffixes estimated providers with `(estimated)` and the inline `Tokens:` readout renders cost as `~$X.XX usage`; the TUI header renders the cost badge with a `~` prefix (`~$X.XX usage`) when any active provider is heuristic; and the NDJSON `cost_updated` / `budget_exceeded` events carry `CostSnapshot.Estimated`. A mixed-backend run (native anthropic + acp estimate) reports the total as estimated with per-provider detail preserved.
