...
| `parallel` | [`pipeline/handlers/parallel.go`](../../pipeline/handlers/parallel.go) | [`handlers/parallel-fan-in.md`](./handlers/parallel-fan-in.md) | Concurrent fan-out over `parallel_targets`. Spawns one goroutine per branch with an isolated context snapshot; records the fan-in join hint on the typed `Outcome.SuggestedNextNodes` field (#451). The engine's `applyOutcome` is the single point that mirrors that slice into the `suggested_next_nodes` context key, which the edge selector reads. |
