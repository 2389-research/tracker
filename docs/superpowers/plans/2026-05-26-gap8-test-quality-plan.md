...
```bash
grep -c 'Emit `STATUS:success` as your terminal line ONLY if INTERFACE' examples/build_product.dip
```
...
in place. Refers to the existing allowlist enumeration earlier
in the SPEC.md compliance section (single source of truth)
rather than re-enumerating filenames inline.
