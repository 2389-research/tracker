...
- **Tool node `timeout` attribute now errors at parse time if set to a zero or negative duration** (closes #151). This is a behavior change. Previously such values reached `context.WithTimeout` and caused immediate cancellation with a confusing "command timed out" error; `ToolHandler.parseTimeout` now returns `node %q has non-positive timeout %q: must be > 0` instead. Pipelines that wrote `timeout: "0"` (unlikely but possible) will now fail to load — configure a positive duration or omit the attr to use the handler default.
