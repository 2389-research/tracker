...
### Added
- **Cloudflare AI Gateway convenience flag** (`TRACKER_GATEWAY_URL` env var, `--gateway-url` CLI flag): Synthesizes per-provider base URLs for the Cloudflare AI Gateway to solve rate limit issues. Provider-specific `*_BASE_URL` env vars take precedence over the gateway URL for per-provider overrides. (#64)
