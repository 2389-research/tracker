...
- **build_product verification gates (#640 D1–D12, E1, E8, B2, B3):** green
  ...
  directory (a Makefile `ci`/`check`/`lint`/`test` target counts as a stack);
  "no build system" is a loud NOTE in the milestone gate (VerifyMilestone
  judges a test-less milestone) and a FAILURE in the ship gate, unless an
  operator placed the opt-out stamp `.ai/build/no-tests-ok` — whose path is
  never printed into agent-visible gate output, and whose creation after a
  milestone started is reported as a finding (D1). The milestone Go scope is base→WORKTREE
  ...
  differed); `PickNextMilestone` snapshots `known_failures` /
  `known_lint_failures` and the operator stamps at milestone start — before
  Implement runs — (`.ai/milestones/<file>.snapshot`, `opt-outs.snapshot`,
  removed by MarkMilestoneDone) and every gate run prints the additions for
  VerifyMilestone (D6). FinalBuild runs
  ...
  a completed red verify (a Ctrl-C/resume mid-verify no longer eats an
  attempt) and reset on the escalate path so a retry gets a fresh budget;
  still exactly 3 red attempts then escalate. `FixMilestone` drops
  `retry_target: TestMilestone`: an engine-level retry (transient provider
  error, `STATUS:retry`, cost/no-progress guard) now re-runs the fixer in
  place — the engine keeps the pipeline context and the working tree — instead
  of re-entering the red gate and burning a fix attempt (B2/B3, #443). Prompts:
