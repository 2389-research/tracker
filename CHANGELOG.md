...
- **Start/exit node handler overwrite broadened fix**: `ensureStartExitNodes` previously checked only three specific attributes (`prompt`, `tool_command`, `mode`) to decide whether to preserve a node's handler. Nodes using other handler types (`parallel` with `parallel_targets`, `parallel.fan_in` with `fan_in_sources`, `conditional`, `subgraph`, `stack.manager_loop`, or `wait.human` with default mode) would still have their handlers silently overwritten. The helper now bases the decision on the resolved `Handler` field: any handler other than `codergen` is always preserved; only a bare `codergen` node with no `prompt` gets the passthrough. Closes #69.
