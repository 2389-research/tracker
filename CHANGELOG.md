...
- **ACP backend surfaces approximate per-prompt token usage** (closes #167). The Agent Client Protocol spec (github.com/coder/acp-go-sdk v0.6.x) has no usage surface — `PromptResponse` carries only `StopReason`+`Meta`, and no `SessionUpdate` subtype reports tokens — so ACP-backed nodes previously returned `SessionResult.Usage` zero-valued. `CodergenHandler.trackExternalBackendUsage` already routes ACP usage to `llm.TokenTracker.AddUsage("acp", ...)`, so the gap was purely in `buildACPResult`. `estimateACPUsage` now synthesizes `llm.Usage` from rune counts (`ceil(runes/4)` with a 1-token floor per non-empty side, applied to `cfg.Prompt`+`cfg.SystemPrompt` as input and the handler's collected agent text as output; UTF-8 aware via `unicode/utf8`), populates `EstimatedCost` via `llm.EstimateCost`, and tags `Usage.Raw` with `ACPUsageMarker{Estimated:true, Source:"acp-chars-heuristic", Ratio:4}`. The tag is visible to consumers that read `SessionResult.Usage` directly; aggregated CLI summaries built via `buildSessionStats` / `pipeline.Trace.AggregateUsage` copy numeric tokens/cost but drop `Usage.Raw`, so an explicit "estimated" flag isn't surfaced in the CLI summary today — plumbing it through the trace schema is tracked as a follow-up. `buildACPResult` now also sets `Provider: "acp"` so per-provider rollups don't bucket ACP sessions under `"unknown"`. A one-line `log.Printf` per ACP session announces the estimate and the approximate totals. Budget guards (`--max-tokens`, `--max-cost`) now enforce against ACP nodes too.
