...
const (
	GateChoice    GateKind = "choice"    // pick one of N labels (radio / buttons)
	GateYesNo     GateKind = "yes_no"    // a fixed Yes/No decision
	GateFreeform  GateKind = "freeform"  // open-ended text reply
	GateInterview GateKind = "interview" // structured multi-question form
)
...
type Gate struct {
	...
	Questions []handlers.Question // GateInterview: the form
}

// GateAnswer carries a human's response back to the blocked pipeline. Exactly
// one field is meaningful per gate kind; Canceled short-circuits all of them.
type GateAnswer struct {
	...
	Interview *handlers.InterviewResult // GateInterview
	Canceled  bool
}
