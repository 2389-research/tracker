...
func parseRunFlags(args []string, cfg runConfig) (runConfig, error) {
	if isHelpAllRequest(args) {
		return cfg, errHelpAll
	}
...
// isHelpAllRequest returns true when the second argument asks for the full
// flag reference. `tracker --help` shows only the common flags; `--help-all`
// (progressive disclosure, #463) prints every flag grouped by concern.
func isHelpAllRequest(args []string) bool {
	if len(args) <= 1 {
		return false
	}
	switch args[1] {
	case "--help-all", "help-all":
		return true
	}
	return false
}
...
func newRunFlagSet(progName string, cfg *runConfig) *flag.FlagSet {
	...
	fs.StringVar(&cfg.gatewayKind, "gateway-kind", "", "Gateway path convention: empty/cf-aig (default) or bedrock (2389 bedrock-gateway Worker)")
