// ABOUTME: Tests for the tracker-conformance CLI binary subcommand dispatch.
...
func TestNoSubcommand(t *testing.T) {
	...
	code := run([]string{"tracker-conformance"}, nil, &stdout, &stderr)
	...
	if !strings.Contains(stderr.String(), "Usage: tracker-conformance <subcommand>") {
		t.Fatalf("expected tracker-conformance usage, got %q", stderr.String())
	}
}

func TestUnknownSubcommand(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"tracker-conformance", "bogus-command"}, nil, &stdout, &stderr)
...
func TestListModels(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"tracker-conformance", "list-models"}, nil, &stdout, &stderr)
...
func TestUnknownSubcommandNotImplemented(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"tracker-conformance", "totally-fake"}, nil, &stdout, &stderr)
