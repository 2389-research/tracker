...
func (s *Session) maybeRunPlanningTurn(ctx context.Context, result *SessionResult) error {
	...
	if resp.Usage.EstimatedCost == 0 {
		result.Usage.EstimatedCost += llm.EstimateCostForProvider(s.pricingProvider(resp), s.pricingModel(resp), resp.Usage)
	}
...
func (s *Session) executeToolCalls(ctx context.Context, toolCalls []llm.ToolCallData, result *SessionResult, turn int) (hadErrors, terminate bool) {
	...
	for _, call := range toolCalls {
		part, isErr, term := s.runOneToolCall(ctx, call, turn, result)
		if isErr {
			hadErrors = true
		}
